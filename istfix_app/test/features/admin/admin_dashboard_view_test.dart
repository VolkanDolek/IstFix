...
void main() {
  ...
  group('AdminDashboardView Widget Testleri', () {
    testWidgets('Sayfa açıldığında API den raporlar çekilmeli ve ekrana çizilmelidir', (tester) async {
      ...
      await tester.pumpAndSettle(); // Yükleme animasyonu bitsin ve liste çizilsin
      ...
      expect(find.text('Bekliyor'), findsOneWidget); // Status çevirisi kontrolü

      expect(find.text('REP-1002'), findsOneWidget);
      expect(find.text('Çözüldü'), findsOneWidget); // Status çevirisi kontrolü
    });

    testWidgets('Arama kutusuna metin yazıldığında liste doğru şekilde filtrelenmelidir', (tester) async {
    ...
    testWidgets('Yönet butonuna tıklandığında dialog açılmalı ve durum güncellenmelidir', (tester) async {
      ...
      await tester.tap(find.widgetWithText(ElevatedButton, 'Değişikliği Kaydet'));
      await tester.pumpAndSettle();
      ...
      expect(find.text('Rapor durumu başarıyla güncellendi!'), findsOneWidget);
    });

    testWidgets('Çıkış yap (Logout) butonuna basıldığında oturum sonlandırılmalı', (tester) async {
      // 1. HAZIRLIK
      when(mockDio.get(any, options: anyNamed('options')))
          .thenAnswer((_) async => _createMockResponse([], 200));
